...
const Director = (props) => (
  <tr>
    <td>
      <img
        src={mainurl + props.director.directorimg}
        alt="directorimg"
        width="100px"
        height="100px"
      />
    ...
    <td>
      ...
      <a
        href="#"
        onClick={() => {
          props.deleteDirector(props.director._id);
        }}
      >
        delete
      </a>
...
export default class DirectorList extends Component {
  ...
  componentDidMount() {
    axios.get(`${mainurl}director/`).then((response) => {
      this.setState({
  ...
  deleteDirector(id) {
    axios.delete(`${mainurl}director/${id}`).then((response) => {
